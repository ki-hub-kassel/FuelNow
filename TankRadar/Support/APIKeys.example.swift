...
enum APIKeys {
    ...
    /// 1. **Simulator (empfohlen):** Auf dem Mac `mkdir -p ~/.tankradar` und eine Zeile Key in  
    ///    `~/.tankradar/tankerkoenig-api-key` speichern. Im Simulator wird über  
    ...
    /// 4. Nur **DEBUG:** UserDefaults-Schlüssel `dev.tankradar.tankerkoenigAPIKey` (z. B. einmalig per Code oder `defaults write`).
    ///
    ...
    private static func resolvedFromSimulatorHostHomeKeyFile() -> String? {
        ...
        let url = URL(fileURLWithPath: hostHome, isDirectory: true)
            .appendingPathComponent(".tankradar", isDirectory: true)
    ...
    private static func resolvedFromUserDefaults() -> String? {
        guard let raw = UserDefaults.standard.string(forKey: "dev.tankradar.tankerkoenigAPIKey") else { return nil }
        return normalizedKey(raw)
    }
    #endif
