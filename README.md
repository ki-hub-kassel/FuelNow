...
**Tankerkönig-API-Key:** Beantragung über Linear [**TAN-72**](https://linear.app/tankradar-app/issue/TAN-72) — landet seit [**TAN-92**](https://linear.app/tankradar-app/issue/TAN-92) **nicht mehr in der App-Binary**, sondern nur noch als verschlüsseltes Vercel-Env-Secret im `tankerkoenig-proxy/`-Worker.

**Default: Vercel-Edge-Function-Proxy (TAN-92).** Die App ruft die Tankerkönig-Pfade `…/api/json/list`, `…/api/json/prices`, `…/api/json/detail` über die im Subfolder [`tankerkoenig-proxy/`](tankerkoenig-proxy/README.md) deployte Vercel-Function auf (bewusst **ohne** `.php`-Suffix, weil Vercel Firewall `.php`-Pfade als Bot-Scan blockt — Antwort `x-vercel-mitigated: deny`). Der Proxy hängt `apikey` serverseitig dran — die App-Binary enthält keinen Key (Tankerkönig-AGB-konform: öffentliche Key-Verbreitung ist dort untersagt). Setup, Deploy und lokales `vercel dev` sind im Proxy-README beschrieben; nach `vercel deploy --prod` landet die URL als Schlüssel `TankerkoenigProxyBaseURL` in `FuelNow/Info.plist`. `TankerkoenigAPIConfiguration.resolved()` schaltet automatisch in den Proxy-Modus, sobald die URL in der Plist (oder Env `TANKERKOENIG_PROXY_BASE_URL` als Override) gesetzt ist. Hosting kostet 0 € (Vercel Hobby, keine Kreditkarte; Limits siehe Proxy-README).

### Checkliste TAN-72 / TAN-92 (betrieblich)

1. Key unter [creativecommons.tankerkoenig.de](https://creativecommons.tankerkoenig.de/) beantragen (Formular, E-Mail bestätigen).
2. Key **nicht** ins Repo: nur als Vercel-Production-Env (`vercel env add TANKERKOENIG_API_KEY production`).
...
5. Smoke (Live-Daten): Live-Tankerkönig ist seit [**TAN-91**](https://linear.app/tankradar-app/issue/TAN-91) **überall Default** (Simulator + Gerät, Debug + Release). Ohne gültigen Key bzw. ohne Verbindung zeigt die App den **Offline-Splash** oder den bestehenden Error-Alert (`missingAPIKey`). Mock-Tankstellen werden **nur noch** mit explizitem `FUELNOW_USE_MOCK_STATIONS=1` geladen (Tests / QA-Snapshots).

**Direct-Modus (Notnagel, ohne Proxy):** wer ohne deployed Proxy testen will, kann den Key alternativ lokal auslegen. Im Simulator: Datei **`~/.fuelnow/tankerkoenig-api-key`** auf dem Mac (`mkdir -p ~/.fuelnow && echo <KEY> > ~/.fuelnow/tankerkoenig-api-key`); `./scripts/build-and-run-simulator.sh` reicht den Wert über `SIMCTL_CHILD_TANKERKOENIG_API_KEY` an `xcrun simctl launch` weiter, sofern in der Plist *kein* Proxy gesetzt ist. Aus Xcode heraus: Run-Scheme → Environment Variables → `TANKERKOENIG_API_KEY` (exakter Name; andere Namen werden nicht gelesen) oder `TANKERKOENIG_API_KEY_FILE` mit absolutem Pfad (siehe Kommentar in `APIKeys.example.swift`).
